...
import ViewCfdiTraslado from "./views/ViewCfdiTraslado";
...
function CfdiTraslado() {
    ...
    const onClickConsult = () => {
        ...
        getShipments(totalRoutes, "CfdiTraslado")
    }
    ...
    return (
        <div className=''>
            <div className='row-auto m-auto bg-zinc-800 w-full p-10 rounded-md'>
                {
                    (mlOperation === "getShipmentsCfdiTraslado") ? (
                ...
                (mlOperation === "getShipmentsCfdiTraslado") ? (
                    (shipments.length !== 0) ? (
                        <div className="row-auto mt-5 m-auto bg-zinc-800 p-5 rounded-md'">
                            <ViewCfdiTraslado routesShipments={shipments} initialDate={dayjs(initialDate).format(formatDate)} finalDate={dayjs(finalDate).format(formatDate)} />
                        </div>
                    ) : null
                ) : null
            }
